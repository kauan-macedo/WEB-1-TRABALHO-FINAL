...
class cursoCont {
	async index(request, response) {
		...
		response.status(200).render('listar_cursos', { cursos: cursos });
	}
	...
	async store(request, response) {
		try {
			...
			response.status(200).send(`<script>alert("Curso adicionado com sucesso!");</script>
					<meta http-equiv="refresh" content="0; url='/cursos/'" />`);
		} catch (error) {
			console.log(error);
			response.send(`<script>alert("Houve algum erro durante a transação. Redirecionando para '/cursos/'");</script>
                    <meta http-equiv="refresh" content="0; url='/cursos/'" />`);
		}
	}
	...
	async update(request, response) {
		...
		const [curso] = await cursoRep.findById(id);
		response.status(200).send(`<script>alert("Curso alterado com sucesso!");</script>
				<meta http-equiv="refresh" content="0; url='/cursos/${id}'" />`);
	}
	...
	async delete(request, response) {
		try {
			...
			console.error(error);
			response.status(500).send(`<script>alert("Erro ao deletar o curso!");</script>
						<meta http-equiv="refresh" content="0; url='/cursos/${id}'" />`);
		}
	}
	...
	async newCurso(request, response) {
		const tiposCursos = await cursoRep.findTipos();
		const turnosCursos = await cursoRep.findTurnos();
		response.status(200).render('criar_cursos', { tipos: tiposCursos, turnos: turnosCursos });
	}
}
